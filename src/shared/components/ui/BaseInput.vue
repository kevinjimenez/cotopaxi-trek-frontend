...
interface Props {
  ...
  customClassContainer?: HTMLAttributes["class"];
  customClassLabel?: HTMLAttributes["class"];
  customClassInput?: HTMLAttributes["class"];
  error?: string;
  required?: boolean;
  helperText?: string;
...
<template>
  <div :class="cn('grid w-full items-center', props.customClassContainer)">
    <label
      v-if="label"
      :class="
        cn('uppercase font-bold text-xs mb-2 text-muted-foreground', customClassLabel, {
